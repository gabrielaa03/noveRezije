...
public class Registration extends AppCompatActivity {
    ...
    public void saveIntoDB() {
        if (Credentials.checkCredentials(imeIPrezime) || Credentials.checkCredentials(adresa) || Credentials.checkCredentials(email)
                ...
                || Credentials.checkCredentials(datumPodsjetnika)) {
            ...
            if (RealmUtils.checkIfUserExists("username", username.getText().toString()) != null) {
                ...
                if (!pass.getText().toString().equals(pass2.getText().toString())) {
                    ...
                    if(Integer.parseInt(datumPodsjetnika.getText().toString())>=1 && Integer.parseInt(datumPodsjetnika.getText().toString())<=31) {
                        Pattern pattern = Pattern.compile(EMAIL_REGEX, Pattern.CASE_INSENSITIVE);
                        Matcher matcher = pattern.matcher(email.getText().toString());
                        if (matcher.matches()) {
                            RealmUtils.saveUser(RealmUtils.createUser(username, imeIPrezime, adresa, email, pass, datumPodsjetnika));
                        ...
                        WidgetUtils.setToast(this, R.string.od1do31);
                    }
                }
            }
        }
